...
public class AssetLocationActivity extends AppCompatActivity {
    ...
    private AssetLocationViewModel viewModel;
    private AssetLocationAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_location_asset);

        taskId = getIntent().getLongExtra("task_id", -1L);
        if (taskId == -1L) taskId = null;
        taskNumber = getIntent().getStringExtra("task_number");

        viewModel = new ViewModelProvider(this).get(AssetLocationViewModel.class);
        ...
        if (taskNumber != null) {
            titleTextView.setText(taskNumber + " - Asset Points");
        }

        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        adapter = new AssetLocationAdapter(this::openScanScreen);
    ...
    private void observeLocations() {
        viewModel.getLocationsState().observe(this, state -> {
            ...
            if (state.getStatus() == Resource.Status.LOADING) {
                ...
                emptyTextView.setText("Failed to load asset points");
            }
        });
    }

    private void openScanScreen(AssetLocationResponse location) {
        Intent intent = new Intent(this, ScanActivity.class);
        intent.putExtra("task_id", taskId);
        intent.putExtra("task_number", taskNumber);
        intent.putExtra("domain", "ASSET");
